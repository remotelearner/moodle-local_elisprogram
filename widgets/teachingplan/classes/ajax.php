...
class ajax {
    ...
    protected function get_filter(array $data) {
        ...
        $table = '\\'.$table;
        if (strpos($table, '\eliswidget_teachingplan\datatable\\') !== 0 || !class_exists($table)) {
    ...
    /**
     * Get a list of class instances (& linked Moodle course) for Course Description.
     *
     ...
     */
    public function get_classesforcourse(array $data) {
        global $DB;
        require_once(\elispm::lib('data/user.class.php'));
        $response = [];
        if (!empty($data['courseid']) && is_numeric($data['courseid'])) {
            $datatable = new \eliswidget_teachingplan\datatable\classes($DB, $this->endpoint);
            $datatable->set_courseid($data['courseid']);
            $datatable->set_userid(\user::get_current_userid());
            $response = $this->get_listing_response($datatable, $data);
        } else {
            throw new \Exception('Invalid Course ID received.');
        }
        return $response;
    }

    /**
     * Get a list of Course Descriptions the user is an instructor in.
     *
     * @param array $data Array containing "filters" and "page".
     * @return array Array of response information containing information on filters to render, search results, and other details
     *               needed by the front-end.
     */
    public function get_coursesforinstructor(array $data) {
        global $DB;
        require_once(\elispm::lib('data/user.class.php'));

        $datatable = new \eliswidget_teachingplan\datatable\courses($DB, $this->endpoint);
        $datatable->set_userid(\user::get_current_userid());
        $datatable->set_progressbar(get_config('eliswidget_teachingplan', 'progressbarenabled'));
        $response = $this->get_listing_response($datatable, $data);
        return $response;
    }
    ...
    protected function get_listing_response(\eliswidget_common\datatable\base $datatable, $data) {
        ...
        $visibledatafields = \eliswidget_teachingplan\datatable\classes::get_visible_tablecolumns();
        $hiddendatafields = [];
        return [
            'filters' => $allfilters,
