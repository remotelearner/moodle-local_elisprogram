 ...
 */
class deepsight_datatable_courseset extends deepsight_datatable_standard {

    /**
     * @var string The main table results are pulled from. This forms that FROM clause.
     */
    protected $main_table = 'local_elisprogram_crsset';
    ...
    protected function get_filters() {
        ...
        $langmodified = get_string('fld_timemodified', 'local_elisprogram');
        ...
        $filters = array(
            ...
            new deepsight_filter_date($this->DB, 'timemodified', $langmodified, array('element.timemodified' => $langmodified)),
        );

        $customfieldfilters = $this->get_custom_field_info(CONTEXT_ELIS_COURSESET);
    ...
    protected function get_fixed_columns() {
        return array(
            'element.name' => get_string('courseset_name', 'local_elisprogram'),
            'element.idnumber' => get_string('courseset_idnumber', 'local_elisprogram')
        );
    }
    ...
    protected function get_join_sql(array $filters = array()) {
        ...
        if (!empty($activecustomfields)) {
            $joinsql[] = 'JOIN {context} ctx ON ctx.instanceid = element.id AND ctx.contextlevel='.CONTEXT_ELIS_COURSESET;
