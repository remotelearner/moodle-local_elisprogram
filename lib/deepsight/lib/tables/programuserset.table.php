...
class deepsight_datatable_programuserset_available extends deepsight_datatable_programuserset_base {
    ...
    protected function get_filter_sql_permissions() {
        ...
        $usassocprgctxs = pm_context_set::for_user_with_capability($ctxlevel, $perm, $USER->id);
        $usassocprgctxsfilerobject = $usassocprgctxs->get_filter('id', $ctxlevel);
        $usassocprgfilter = $usassocprgctxsfilerobject->get_sql(false, 'element', SQL_PARAMS_QM);
        if (isset($usassocprgfilter['where'])) {
            $additionalfilters[] = $usassocprgfilter['where'];
            $additionalparams = array_merge($additionalparams, $usassocprgfilter['where_parameters']);
