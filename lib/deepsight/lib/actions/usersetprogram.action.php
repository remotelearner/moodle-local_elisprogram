            ...
            if ($this->can_assign($usersetid, $programid) === true) {
                ...
                if ($recursive) {
                    ...
                    foreach ($subsets as $subset) {
                        if ($this->can_assign($subset->id, $programid) === true) {
        ...
        foreach ($elements as $programid => $label) {
            if ($this->can_edit($usersetid, $programid) === true) {
                ...
                if ($recursive) {
                    $subsets = $userset->get_all_subsets();
                    foreach ($subsets as $subset) {
                        if ($this->can_edit($subset->id, $programid) === true) {
 ...
 */
class deepsight_action_usersetprogram_unassign extends deepsight_action_standard {
    ...
    protected function _respond_to_js(array $elements, $bulkaction) {
        ...
        foreach ($elements as $programid => $label) {
            if ($this->can_unassign($usersetid, $programid) === true) {
                ...
                if (($assignrec = $DB->get_record(clustercurriculum::TABLE, $assignrecfilters))) {
                ...
                if ($recursive) {
                    $subsets = $userset->get_all_subsets();
                    foreach ($subsets as $subset) {
                        if ($this->can_unassign($subset->id, $programid) === true) {
                            $assignrecfilters = array('clusterid' => $subset->id, 'curriculumid' => $programid);
                            if (($assignrec = $DB->get_record(clustercurriculum::TABLE, $assignrecfilters))) {
                                $clustercurriculum = new clustercurriculum($assignrec);
                                $clustercurriculum->delete();
                            }
