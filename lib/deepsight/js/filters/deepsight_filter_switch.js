...
$.fn.deepsight_filter_switch = function(options) {
    ...
    this.initialize = function() {
        ...
        main.filterui = $('<button></button').addClass('filterui').addClass('emphasis');
        main.filterui.html(opts.label+' '+opts.choices[0].label);
        main.append(main.filterui);

        main.filterui.attach_dropdown();
        main.filterui.dropdown.addClass('emphasis');
        // Initial values are passed as arrays.
        if (typeof opts.initialvalue !== 'undefined' && opts.initialvalue.length === 1) {
            val = opts.initialvalue[0];
        }
