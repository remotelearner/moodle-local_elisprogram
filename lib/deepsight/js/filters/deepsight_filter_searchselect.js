...
$.fn.deepsight_filter_searchselect = function(options) {
    ...
    this.render_choice = function(id, label, checked) {
        ...
        if (/[^a-z^A-Z^0-9]/.test(id)) {
            newid = id.replace(/[^a-z^A-Z^0-9]/g, '_');
        }
    ...
    this.initialize = function() {
        ...
        if (typeof(opts.initialvalue) != 'undefined') {
            for (var i in opts.initialvalue) {
                if (initialval != '') {
                    initialval += ',';
                }
                flag = false;
                for (var j in opts.initialchoices) {
                    if (opts.initialvalue[i] == opts.initialchoices[j].id) {
                        flag = true;
                        initialval += opts.initialchoices[j].label;
                        main.selections[opts.initialvalue[i]] = {id: opts.initialvalue[i], label: opts.initialchoices[j].label};
                        opts.datatable.filter_add(main.name, opts.initialvalue[i]);
                    }
                }
                if (!flag) {
                    initialval += opts.initialvalue[i].toString();
                }
            }
        }
