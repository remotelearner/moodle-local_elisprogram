...
/** @var array lepcrsactivelist is an array of table ids for active courses */
var lepcrsactivelist = [];
 ...
 *
 * Usage:
 *     $('[button selector]').each(function() { $(this).deepsight_action_crssetcrs(); });
 *
 * Required Options:
 ...
 */
$.fn.deepsight_action_crssetcrs = function(options) {
    this.default_opts = {
        rowdata: null,
        parent: null,
        sesskey: null,
    ...
    this.render_action = function() {
        ...
        if (opts.parentid != 'bulklist' && !options.rowdata.meta.candel) {
            ...
            if (opts.parentid == 'bulklist') {
                var hasactive = false;
                if (window.lepcrsactivelist.length) {
                    var bulkid;
                    for (bulkid in opts.parent.prevObject.selected_elements_page_ids) {
                        if (window.lepcrsactivelist.indexOf(opts.parent.prevObject.selected_elements_page_ids[bulkid].toString()) != -1) {
                            hasactive = true;
                            break;
                        }
                    }
