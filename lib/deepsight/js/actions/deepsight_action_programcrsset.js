...
/** @var array lepcrssetactivelist is an array of table ids for active coursesets required for bulk list to check to give special confirmation message */
var lepcrssetactivelist = [];

(function($) {

/**
 * DeepSight CourseSet-Program Action Panel
 * Adds a confirmation action panel that can be used to ensure the user actually wants to perform the action.
 *
 * Usage:
 *     $('[button selector]').each(function() { $(this).deepsight_action_programcrsset(); });
 *
 * Required Options:
 ...
 *     string actiontrclass     A Css class to attach the action's parent.
       string langbulkconfirm   The confirmation message to display when applying the action to the bulklist.
 ...
 */
$.fn.deepsight_action_programcrsset = function(options) {
    this.default_opts = {
        rowdata: null,
        parent: null,
        sesskey: null,
        ...
        desc_single_active: 'Are you sure?',
        desc_multiple: 'Are you sure?',
        desc_multiple_active: 'Are you sure?',
        wrapper: '<div></div>',
        actionclass: 'deepsight_actionpanel',
        actiontrclass: 'deepsight_actionpanel_tr',
        langbulkconfirm: 'Performing actions on many users can take some time - Are you sure?',
        langbulkconfirmactive: 'Performing actions on many users can take some time - Are you sure?',
        langworking: 'Working...',
    ...
    this.render_action = function() {
        var desc;
        if (opts.parentid != 'bulklist' && !options.rowdata.meta.candel) {
            desc = opts.no_permission;
            var actionpanelbody = '<div class="body">'+desc+'</div>';
        } else {
            if (opts.parentid == 'bulklist') {
                var hasactive = false;
                if (window.lepcrssetactivelist.length) {
                    var bulkid;
                    for (bulkid in opts.parent.prevObject.selected_elements_page_ids) {
                        if (window.lepcrssetactivelist.indexOf(opts.parent.prevObject.selected_elements_page_ids[bulkid].toString()) != -1) {
                            hasactive = true;
                            break;
                        }
                    }
                }
                desc = hasactive ? opts.langbulkconfirmactive : opts.desc_multiple;
            } else {
                desc = options.rowdata.meta.isactive ? opts.langconfirmactive : opts.desc_single;
            }
            var actionpanelbody = '<div class="body">'+desc+'</div>\n\
