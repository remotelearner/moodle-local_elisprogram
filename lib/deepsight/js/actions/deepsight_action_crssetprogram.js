...
/** @var array lepprgactivelist is an array of table ids for active programs required for bulk list to check to give special confirmation message */
var lepprgactivelist = [];
 ...
 *
 * Usage:
 *     $('[button selector]').each(function() { $(this).deepsight_action_crssetprg(); });
 *
 * Required Options:
 ...
 */
$.fn.deepsight_action_crssetprg = function(options) {
    this.default_opts = {
        rowdata: null,
        parent: null,
        sesskey: null,
    ...
    this.render_action = function() {
        ...
        if (opts.parentid != 'bulklist' && !options.rowdata.meta.candel) {
            ...
            if (opts.parentid == 'bulklist') {
                var hasactive = false;
                if (window.lepprgactivelist.length) {
                    var bulkid;
                    for (bulkid in opts.parent.prevObject.selected_elements_page_ids) {
                        if (window.lepprgactivelist.indexOf(opts.parent.prevObject.selected_elements_page_ids[bulkid].toString()) != -1) {
                            hasactive = true;
                            break;
                        }
                    }
