...
/**
 * DeepSight Userset/Subset Delete
 * An action used in userset/subset deletion.
 *
 * Usage:
 *     $('[button selector]').each(function() { $(this).deepsight_action_usersetsubset_delete(); });
 *
 * Required Options:
 ...
 *     string langdeletesubs    The language string to display beside the delete subsets radio button.
 *     string langpromotesubs   The language string to display beside the promote subsets radio button.
 ...
 */
$.fn.deepsight_action_usersetsubset_delete = function(options) {
    this.default_opts = {
        rowdata: {},
        parent: null,
        sesskey: null,
        ...
        desc_single: 'You are about to delete this userset. Continue?',
        desc_multiple: 'You are about to delete theses usersets. Continue?',
        mode: 'assign', // TBD
        lang_bulk_confirm: 'Bulk actions can take some time, are you sure you want to continue?',
        lang_working: 'Working...',
        langdeletesubs: 'langdeletesubs',
        langpromotesubs: 'langpromotesubs',
        langyes: 'Yes',
        langno: 'No'
    }
    ...
    this.render_action = function() {
        ...
        // Assemble form.
        var subsetsid = 'deepsight_action_usersetsubset_delete_subsets'; // TBD: bulk?
        if (opts.parentid != 'bulklist') {
            subsetsid += '_'+opts.rowdata.element_id;
        }
        var form = '<form>';
        form += '<input type="radio" class="subsets" name="subsets" id="'+subsetsid+'_0" value="0" checked="checked">'+opts.langpromotesubs+'</input>';
        form += '<br/><input type="radio" class="subsets" name="subsets" id="'+subsetsid+'_1" value="1">'+opts.langdeletesubs+'</input>';
        form += '</form>';

        var actionpanel = $('<div><div>')
            .addClass(opts.actionclass)
            .addClass('deepsight_action_usersetsubset_delete')
            .css('display', 'none');
    ...
    this.precomplete_action = function(e) {
        var subsets = 0;
        var subsetsradio = main.form.find('input.subsets');
        if (subsetsradio.length > 1) {
            subsets = (subsetsradio[1].checked == true) ? 1 : 0;
        }
        if (opts.parentid == 'bulklist' && typeof(bulkconfirmed) == 'undefined') {
            ...
            main.actiontr.find('i.elisicon-confirm')
                ...
                .bind('click', function(e) { main.complete_action(e, subsets); });
        } else {
            main.complete_action(e, subsets);
        }
    }
     ...
     *
     * @param object e The jquery event object that initialized the completion.
     * @param int subsets The value of the subsets radio buttons to delete or promote subsets.
     */
    this.complete_action = function(e, subsets) {
